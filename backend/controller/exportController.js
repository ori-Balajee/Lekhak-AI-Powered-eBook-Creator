    ...
    for(let i=0; i<tokens.length; i++){
        ...
        try{
            if(token.type === "heading_open"){
                ...
                listType = null;
                orderedCounter = 1;
                paragraphs.push(new Paragrapgh({text:"", spacing: {after: 100}}));
            ...
            }else if(token.type === "blockqoute_open"){
                const nextToken = tokens[i+1];
...
const renderInlineTokens = (doc, tokens,options = {}) =>{
    ...
    const flushBuffer = () =>{
        if(textBuffer){
            doc.text(currentFont).text(textBuffer, {
               ...baseOptions,
               continued: true,
            });
            textBuffer = "";
    ...
    for(let i=0; i<tokens.length; i++){
        ...
        if(token.type === "text"){
            ...
            flushBuffer();
            currentFont = TYPOGRAPHY.fonts.serifItalics;
    ...
    for(let i=0; i<tokens.length; i++){
        ...
        try{
            if(token.type === "heading_open"){
                ...
                doc
                    .font("Courier")
                    .fontSize(TYPOGRAPHY.colors.text)
                    .text(token.content, {
                        indent: 20,
                        align: "left",
                    });
        ...
        if(book.chapters && book.chapters.length > 0){
            book.chapters.forEach((chapter,index) =>{
                try{
                    ...
                    console.errror(`Error processing chapter ${index}:`, chapterError);
                }
            });
        }
